...
partial class Map : ComponentBase, IDisposable
{
    private Menu _menu;
    ...
    private Task<SchoolData[]> _getSchoolsTask;
    ...
    private async Task LoadSchools()
    {
        ...
        _schoolsReady = true;
        await InvokeAsync(StateHasChanged); // triggers re-render
    }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        if (_mapRendered && _schoolsReady && _schools != null)
        {
            _schoolsReady = false; // prevent re-running on subsequent renders
